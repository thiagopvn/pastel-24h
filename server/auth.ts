  ...
  if (stored.startsWith('$scrypt$')) {
    ...
    try {
      ...
      const derivedKey = await new Promise<Buffer>((resolve, reject) => {
        scrypt(supplied, parsed.salt, keyLength, {
          N: parsed.N,
          r: parsed.r,
          p: parsed.p,
        }, (err, key) => {
...
export function setupAuth(app: Express) {
  // Garante que a chave secreta existe e tem um tamanho mínimo para segurança
  if (!process.env.SESSION_SECRET || process.env.SESSION_SECRET.length < 32) {
    throw new Error("SESSION_SECRET must be set in the .env file and be at least 32 characters long.");
  }
